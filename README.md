...
| 📥 **下载中心** | 左侧导航栏新增“下载中心”，包含“批量下载”（全部历史 / 当前月份 / 收藏 / 日历式日期范围选择）与“已下载的壁纸”（本地壁纸目录缩略图画廊，显示图片分辨率与文件大小，点击图片可预览大图并直接设为桌面壁纸，支持单个或勾选批量删除）两个子页面；关键按钮带悬浮提示，顶部状态提示会自动截断，不遮挡右侧操作按钮。 |
...
| 🆕 **一键检测更新** | 启动时自动检测 Gitee / GitHub Releases 是否有新版本发布，设置面板也可手动检查；更新包下载优先使用 Gitee 国内 Release 附件，失败时自动回退 GitHub 官方附件，完成后自动重启升级。 |
