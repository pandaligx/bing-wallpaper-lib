...
| ⬇️ **高速下载引擎** | 基于 [aria2](https://github.com/aria2/aria2) 的 JSON-RPC 接口，多连接分片、不限速，下载时有实时进度条，并支持批量下载全部历史、当前月份、收藏壁纸或指定日期范围。 |
| 🖥️ **一键设置桌面壁纸** | 通过 Windows 桌面壁纸 API 设置壁纸，支持同步全部显示器或只设置某一个显示器。 |
| ❤ **我的收藏** | 左侧导航栏新增“我的收藏”，首页和归档列表可用固定尺寸心形图标收藏/取消收藏壁纸。 |
| ⚙️ **设置浮层** | 左下角设置浮层使用悬停右侧展开的二级面板，可配置下载路径、外观模式、多显示器壁纸目标、批量下载和维护功能。 |
...
<p align="center">
  ...
  <br/>
  <sub>首页网格视图：默认展示最近壁纸，支持无限滚动加载更多；左侧按年 / 月归档，左下角设置浮层通过悬停右侧展开二级面板</sub>
</p>
