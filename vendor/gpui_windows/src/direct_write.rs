...
struct DirectWriteComponents {
    ...
    in_memory_loader: crate::font_loader::FontLoader,
    builder: IDWriteFontSetBuilder,
...
impl Drop for DirectWriteComponents {
    fn drop(&mut self) {
        unsafe {
            let _ = self
                .factory
                .UnregisterFontFileLoader(&self.in_memory_loader.interface);
        }
    }
}
...
impl DirectWriteTextSystem {
    pub(crate) fn new(directx_devices: &DirectXDevices) -> Result<Self> {
        ...
        let in_memory_loader = crate::font_loader::FontLoader::new();
        unsafe { factory.RegisterFontFileLoader(&in_memory_loader.interface)? };
    ...
    fn add_fonts(
        ...
        for font_data in fonts {
            let font_file = components
                .in_memory_loader
                .add_file(&components.factory, &font_data)?;
            unsafe {
                let mut supported = BOOL(0);
                let mut file_type = DWRITE_FONT_FILE_TYPE_UNKNOWN;
                let mut face_count = 0;
                font_file.Analyze(&mut supported, &mut file_type, None, &mut face_count)?;
                anyhow::ensure!(supported.as_bool(), "Unsupported custom font format");
                for index in 0..face_count {
                    let face = components.factory.CreateFontFaceReference(
                        &font_file,
                        index,
                        DWRITE_FONT_SIMULATIONS_NONE,
                    )?;
                    components.builder.AddFontFaceReference2(&face)?;
                }
            }
        }
        ...
        while unsafe { color_enumerator.MoveNext()? }.as_bool() {
            ...
            let color_run = unsafe { &*color_run };
            {
                ...
                if color_size.width > 0 && color_size.height > 0 {
                    ...
                    let run_color = {
                        let run_color = color_run.runColor;
                        Rgba {
                            r: run_color.r,
                            g: run_color.g,
                            b: run_color.b,
