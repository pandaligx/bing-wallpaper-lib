...
impl WindowsWindowInner {
    ...
    fn handle_calc_client_size(
        ...
        unsafe {
            ...
            if self.state.is_maximized() {
                let dpi = crate::compat::get_dpi_for_window(handle);
    ...
    fn handle_hit_test_msg(&self, handle: HWND, lparam: LPARAM) -> Option<isize> {
        ...
        let dpi = unsafe { crate::compat::get_dpi_for_window(handle) };
...
fn get_frame_thicknessx(dpi: u32) -> i32 {
    let resize_frame_thickness =
        unsafe { crate::compat::get_system_metrics_for_dpi(SM_CXSIZEFRAME, dpi) };
    let padding_thickness =
        unsafe { crate::compat::get_system_metrics_for_dpi(SM_CXPADDEDBORDER, dpi) };
    resize_frame_thickness + padding_thickness
}

fn get_frame_thicknessy(dpi: u32) -> i32 {
    let resize_frame_thickness =
        unsafe { crate::compat::get_system_metrics_for_dpi(SM_CYSIZEFRAME, dpi) };
    let padding_thickness =
        unsafe { crate::compat::get_system_metrics_for_dpi(SM_CXPADDEDBORDER, dpi) };
    resize_frame_thickness + padding_thickness
}
