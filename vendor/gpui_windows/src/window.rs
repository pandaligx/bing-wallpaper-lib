...
pub struct WindowsWindowState {
    ...
    show_after_first_present: Cell<bool>,
    hwnd: HWND,
...
impl WindowsWindowState {
    fn new(
        ...
        let scale_factor = {
            let monitor_dpi = unsafe { crate::compat::get_dpi_for_window(hwnd) } as f32;
        ...
        Ok(Self {
            ...
            show_after_first_present: Cell::new(false),
            hwnd,
            invalidate_devices,
            direct_manipulation,
...
impl WindowsWindow {
    pub(crate) fn new(
        ...
        // Establish the final client size while hidden. Showing the native window
        // before its first swap-chain presentation exposes an empty white frame.
        ...
        this.state.show_after_first_present.set(params.show);
...
impl PlatformWindow for WindowsWindow {
    ...
    fn on_request_frame(&self, callback: Box<dyn FnMut(RequestFrameOptions)>) {
        ...
        if self.state.show_after_first_present.get() {
            // Hidden windows do not reliably receive WM_PAINT. Post a frame
            // request so it runs after GPUI has installed the root view.
            unsafe {
                PostMessageW(
                    Some(self.hwnd),
    ...
    fn draw(&self, scene: &Scene) {
        let presented = self
            ...
            .log_err();
        if presented.is_some() && self.state.show_after_first_present.replace(false) {
            let this = self.0.clone();
            // Defer native visibility changes until GPUI releases the draw lock:
            // SetWindowPlacement can synchronously dispatch size/focus messages.
            self.executor
                .spawn(async move {
                    this.set_window_placement().log_err();
                })
...
mod tests {
    ...
    fn hidden_window_placement_applies_size_without_showing_a_frame() {
        ...
        unsafe {
            ...
            SetWindowPlacement(window.0, &placement).unwrap();
