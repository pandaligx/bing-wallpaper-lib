...
- 历史归档主数据源：[zxyongyo/bing-daily-wallpaper](https://github.com/zxyongyo/bing-daily-wallpaper) 的 `map.json`。
  ...
  `url_preview`、`url_1080`、`url_4k` 等。本项目将 `url_4k` 保存为 `WallpaperEntry::url` 作为默认 4K
  地址；实际下载/设置壁纸时通过 `WallpaperEntry::download_url(settings.download_resolution)` 按全局分辨率
  生成最终地址。`title` 保存为 `WallpaperEntry::headline`，用于卡片标题和下载文件名前缀。
...
- `2024-01-26` 的北鹰鸮记录曾存在但三条 URL 的 OHR 标识被错误拼接，现已在 corrections 文件中用
  `OHR.HawkOwl_ZH-CN3401920167` 单独覆盖，并由 `bundled_snapshot_is_parseable` 回归测试锁定。
- corrections 文件中的记录会在读取旧缓存和合并远程归档后再次覆盖同日记录；这保证已经存在于缓存或上游中的
  错误 URL（例如 `2026-07-08`）也会被修正，而不只是补回完全缺失的日期。
- `2024-09-17` 与 `2025-11-19` 曾从上游归档中缺失，`2026-07-08` 的阿蒂特兰湖记录则曾把分辨率数字错误拼入
  OHR 标识；三条记录现均由 corrections 文件持续覆盖，并由 `bundled_snapshot_is_parseable` 回归测试锁定日期、
  标题、OHR 标识与 4K 参数，避免后续自动同步重新引入缺口或坏链接。
...
1. **缩略图性能**：已在 `WallpaperEntry::thumbnail_url()`（`src/model.rs`）中实现——对带有 `w=3840&h=2160`
   ...
   分辨率，原样返回。下载/设为壁纸时使用 `WallpaperEntry::download_url(...)` 按全局分辨率生成高清地址。
   自 v0.2.29 起，首页网格和
   月份列表的远程缩略图还会经过 `BoundedImageCache` LRU 缓存，最多保留最近使用的 120 张缩略图，旧图片会
   调用 `cx.drop_image(...)` 主动释放，避免长距离滚动后内存持续累积。
...
`src/i18n.rs` 定义 `LanguagePreference` 与内置翻译表，支持跟随 Windows 系统语言、English、简体中文、
日本語、한국어、Русский、Français；不支持的系统语言回退到英文。选择保存在 `AppSettings::language`，
运行时切换会同时调用 `gpui_component::set_locale`，使组件库自带的日期选择器等控件同步更新语言。
左侧栏底部设置按钮右侧的国旗按钮负责打开语言菜单。仓库默认主页 `README.md` 使用英文，
`README.zh-CN.md` 提供简体中文版本，两者顶部互相链接。
...
- **预览弹窗**（`WallpaperLibrary::open_preview_dialog`）：展示原始高清大图
  的 1080p 预览版本（`entry.preview_url()`，不是缩略图；下载/设为桌面壁纸仍使用
  `entry.preview_url()` 的 1080p 预览地址；下载/设为桌面壁纸仍走 `entry.download_url(...)`）+ 标题，底部 `DialogFooter` 提供"下载"/"设为
  桌面壁纸"按钮，点击后复用已有的 `start_download`/`set_as_wallpaper` 方法
  并立即 `window.close_dialog(cx)`。自 v0.2.29 起，预览弹窗内部单独创建
  `RetainAllImageCache`，只包住当前预览图；弹窗关闭后缓存实体释放，触发 GPUI 的
  `observe_release` / `cx.drop_image(...)` 清理图片资源。
...
### 14.3 自动检测 Gitee / GitHub 新版本 + 一键下载升级
...
- `check_for_update(http)` 优先请求 Gitee 公开 REST API
  `GET https://gitee.com/api/v5/repos/pandaligx/bing-wallpaper-lib/releases/latest`，解析 `tag_name`
  （如 `v0.2.0`）与当前版本比较，并通过
  `GET /v5/repos/{owner}/{repo}/releases/{release_id}/attach_files` 找到第一个 `.exe` 结尾的附件作为
  下载目标；若 Gitee Release 不可用，则回退 GitHub 公开 REST API
  `GET https://api.github.com/repos/pandaligx/bing-wallpaper-lib/releases/latest`。返回 `Ok(None)` 表示
  已是最新。
...
## 17. 更新包下载源演进：手动直链 → Gitee/GitHub Release 双源

历史上 v0.2.10 起曾使用 `updater.rs::UPDATE_MIRROR_URL_TEMPLATE` 指向
`lgxng.cn` 的手动上传直链，用于避开 GitHub Release asset 在国内下载不稳定的问题。v0.2.17 又为该方案补了
`fallback_download_url: Option<String>`，让“直链镜像失败 → GitHub 官方地址”可以自动重试，避免镜像站 504、
超时或文件未同步时直接升级失败。

当前维护方案已经废弃手动直链模板：`fallback_download_url` 仍保留，但它的语义变为“Gitee Release 附件失败
→ GitHub Release 附件”。`check_for_update` 会优先请求 Gitee Releases latest 并读取同 release 的
`attach_files`；若 Gitee Release 缺失、接口不可用或附件不存在，再回退 GitHub Releases。`ui/mod.rs::run_update_download`
仍按顺序尝试 `download_url` 与 `fallback_download_url`，每次重试前会清理更新目录中同名 exe 与 `.aria2`
控制文件，并重置进度条；备用地址重试时状态栏提示“国内发行版下载失败，正在尝试备用下载地址...”。

**重要发布提醒**：对于已经发布的 v0.2.10~v0.2.16，它们仍然会按旧逻辑优先且只访问当时的直链 URL。
当前代码只能保护后续版本；若需要旧版本用户无缝升级到新版本，仍需考虑这些旧版本内置逻辑的限制。
...
## 24. 设置面板关闭按钮与 tooltip 穿透修复（v0.2.26）
...
## 26. 图片预览与滚动缓存内存优化（v0.2.29）

v0.2.29 针对 GPUI 图片资源缓存做了两层治理：

1. `WallpaperEntry::preview_url()` 将预览弹窗使用的图片地址从 4K (`w=3840&h=2160`) 转为 1080p
   (`w=1920&h=1080`)。下载与设置桌面壁纸使用 `WallpaperEntry::download_url(...)` 按用户选择的全局分辨率生成地址。
2. `WallpaperLibrary::open_preview_dialog` 为每个远程预览弹窗创建独立 `RetainAllImageCache`，只包住当前大图预览。
   弹窗关闭后该缓存实体释放，GPUI 会通过 `observe_release` 调用 `cx.drop_image(...)` 清理图片资源。
3. 首页网格和月份列表的远程缩略图统一走 `BoundedImageCache`。该缓存实现了 `gpui::ImageCache`，按 LRU 策略最多
   保留 `THUMBNAIL_IMAGE_CACHE_ITEMS`（当前 120）张缩略图；超过上限时会移除最久未使用的图片并调用
   `cx.drop_image(image, Some(window))` 主动释放纹理。
4. `WallpaperLibrary::should_close_window` 以及“退出选择”弹窗中的“最小化到托盘”路径都会先调用
...
v0.2.30 新增 `settings.rs::DownloadResolution`，并作为 `AppSettings::download_resolution` 持久化到
`settings.json`，默认值为 `FourK`。左侧导航栏的“全局分辨率”菜单由 `ui/mod.rs` 渲染，当前提供：

- `Original`：显示为“原图 UHD”，通过 `WallpaperEntry::download_url(Original)` 截取无查询参数的 `_UHD.jpg`
  原始地址，例如 `https://cn.bing.com/th?id=OHR.X_UHD.jpg`。
- `FourK`：显示为“4K 3840×2160”，保持 `WallpaperEntry::url` 中归档提供的默认 4K 地址。
- `TwoK`：显示为“2K 2560×1440”，优先把 URL 查询参数改为 `w=2560&h=1440`。
- `OneK`：显示为“1K 1920×1080”，优先把 URL 查询参数改为 `w=1920&h=1080`。

单张下载、批量下载、每日自动壁纸和“一键设置桌面壁纸”都必须走 `WallpaperEntry::download_url(...)`，
不要再直接把 `entry.url` 传给 aria2，否则会绕过全局分辨率设置。预览弹窗仍使用 `preview_url()` 的 1080p
地址，以控制 GPUI 图片缓存占用。
...
- Release 附件名必须匹配 `bing-wallpaper-lib-vX.Y.Z-x64.exe` 的安全字符集合；下载后必须是普通文件、至少
  1 MiB 且以 `MZ` 开头。
- 自 v0.2.37 起，自更新保留原目录并使用新版发行文件名；清理旧 exe 前先迁移开机自启注册表和计划任务路径。用户自建且指向旧文件名的快捷方式需自行更新；
  批处理路径中的 `%` 必须转义为 `%%`。
...
## 30. 2024-01-26 归档纠错与多语言界面（v0.2.33）

`2024-01-26` 北鹰鸮记录虽然存在于上游数据中，但 OHR 标识和分辨率参数被错误拼接。该日期现在通过
`assets/data/zxyongyo-bing-wallpaper-corrections.json` 独立覆盖为
`OHR.HawkOwl_ZH-CN3401920167`，自动同步不得删除或还原此修复。

`src/i18n.rs` 维护界面语言偏好、Windows 系统语言识别和应用内翻译表。语言选择通过
...
- `src/window_sizing.rs` 区分 Win32 物理像素与 GPUI 逻辑像素，根据主屏有效 DPI 缩放目标尺寸、预留边框空间并在工作区居中；托盘恢复继续使用物理像素。回归测试覆盖 100%～300% 和小屏工作区。
- 设置和语言浮层的面板及全屏遮罩使用 `.occlude()` 阻断下层鼠标命中；仅停止点击传播无法阻止导航高亮或 tooltip。组件库自带 Dialog/Popover 已提供相应遮挡。
- 更新采用新版发行文件名；新程序以 `--repair-update-paths` 迁移已启用的开机自启及计划任务后，再启动界面并清理旧 exe。迁移失败保留旧文件，并带 `--skip-update-filename-migration` 回退，避免循环重启。
- `finish_legacy_update` 兼容旧更新器把新版覆盖到旧版文件名的情况；普通启动时不改动用户自定义文件名。发布时需同步 Cargo 与资源版本，检查成功更新和迁移失败回退两条路径。