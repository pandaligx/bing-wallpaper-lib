...
`SharedString` 常量（`COPYRIGHT`）形式在 `src/ui/mod.rs` 中展示于侧边栏底部（`SidebarFooter`）。
...
`WM_SETTINGCHANGE`/主题相关消息），回调中再次调用 `Theme::sync_system_appearance`，实现用户在系统设置中
切换深色/浅色时，无需重启应用即可自动跟随。
...
### 12.3 预览大图 / 设置面板：基于 `gpui_component::dialog` 的弹窗
...
- 侧边栏头部内置了现成的 `SidebarToggleButton`（自带
  ...
  内部动画完成。
- 收起状态下，侧边栏头部的软件名称/副标题、底部的版权文字都通过
  `.when(!sidebar_collapsed, |this| ...)` 隐藏，只保留图标按钮，避免窄栏下
  文字挤压换行。

### 12.5 设置系统与自定义下载路径

新增 `src/settings.rs`：`AppSettings { download_dir: Option<PathBuf> }`，
...
- 出于安全考虑（避免与 GPUI/Windows 平台层可能存在的 COM 初始化方式冲突），
  没有使用 `IFileOpenDialog`/`SHBrowseForFolderW` 等原生文件夹选择对话框，
  而是提供一个文本输入框 + "在资源管理器中打开"按钮的组合，让用户自己
  粘贴/输入路径，足够满足"手动设置下载路径"的需求且实现风险最低。
...
| `src/model.rs` | 数据模型与按年月分组，缩略图 URL 转换 |
