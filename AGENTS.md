...
1. **缩略图性能**：已在 `WallpaperEntry::thumbnail_url()`（`src/model.rs`）中实现——对带有 `w=3840&h=2160`
   ...
   分辨率，原样返回。下载/设为壁纸时仍使用 `WallpaperEntry::url` 原始高清地址。自 v0.2.29 起，首页网格和
...
- **预览弹窗**（`WallpaperLibrary::open_preview_dialog`）：展示原始高清大图
  ...
  `entry.url` 原始 4K 地址）+ 标题，底部 `DialogFooter` 提供"下载"/"设为
...
1. `WallpaperEntry::preview_url()` 将预览弹窗使用的图片地址从 4K (`w=3840&h=2160`) 转为 1080p
   (`w=1920&h=1080`)。下载与设置桌面壁纸仍使用 `WallpaperEntry::url` 原始 4K 地址，因此不会降低壁纸质量。
